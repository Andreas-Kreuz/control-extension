...
describe("Lane ...", function ()
    local function seedTrainRoute(trainName, route)
        require("ce.hub.data.trains.TrainRegistry").seedFromSnapshot({ name = trainName, route = route })
    end
    ...
    insulate("Approach compatibility", function ()
        ...
        it("exposes stable structured request facts", function ()
            local signal = TrafficLight:new("K1", 71, TrafficLightModel.Unsichtbar_2er)
            local lane = Lane:new("Lane A", signal)

            lane:vehicleEntered("#Car1")
            local state = lane:getRequestState()

            assert.equals(signal, lane:getLaneSignal())
            assert.is_true(state.occupied)
            assert.equals(1, state.vehicleCount)
            assert.equals(0, state.waitCount)
            assert.equals(Lane.RequestType.NORMAL, state.requestType)
            assert.equals("counter", state.source)
            assert.are.same({ "#Car1" }, state.queuedVehicleNames)
            assert.are.same(state, lane:getRequestState())
        end)
    ...
    insulate("Route lookup", function ()
        ...
        it("uses the default route when no hub train exists", function ()
            local Lane, TrafficLight, TrafficLightModel, routeStub, routeCalls = setup()
            ...
            assert.equals("Alle", firstVehiclesRoute)
            assert.equals(0, calls)
        end)
    end)
    insulate("Register signals", function ()
        ...
        seedTrainRoute("#Car1", "Some Route")
    ...
    insulate("Can drive on route", function ()
        ...
        seedTrainRoute("#Car1", "Matching Route")
        -- Signal which is visible to tell the lanes traffic to drive
    ...
    insulate("Show requests on the correct signal groups", function ()
        ...
        seedTrainRoute("#Car1", "Route A")
        seedTrainRoute("#Car2", "Route B")
        seedTrainRoute("#Car3", "Route C")
    ...
    describe("Tag text loading", function ()
        insulate("No queued vehicles, but a counter", function ()
            ...
            end)            insulate("a matching vehicle behind the first good vehicle is removed", function ()
                local signal = TrafficLight:new("Y", 56, TrafficLightModel.Unsichtbar_2er)
                local currentLane = Lane:new("Lane B", signal)
                currentLane:vehicleEntered("train 1")
                currentLane:vehicleEntered("train 2")
                currentLane:vehicleEntered("train 3")
                currentLane:vehicleLeft("train 2")

                it("Queue keeps only vehicles after the matching leave", function ()
                    assert.are.same({ "train 3" }, currentLane.queue:elements())
                end)
                it("Lane vehicle count follows the queue", function () assert.equals(1, currentLane.vehicleCount) end)
            end)
        end)
    end)
    ...
    describe(":useSignalForQueue()", function ()
        ...
        insulate("with train", function ()
            ...
            seedTrainRoute("#Car1", "Some Route")
            seedTrainRoute("#Car2", "Some Route")
    ...
    insulate("Signal tag persistence", function ()
        ...
        it("shrinks queued vehicle names until the lane signal tag fits", function ()
            local firstName = "#" .. string.rep("First", 30)
            local secondName = "#" .. string.rep("Second", 30)
            local thirdName = "#" .. string.rep("Third", 300)
            local laneSignal = TrafficLight:new("K4", 169, TrafficLightModel.Unsichtbar_2er)
            local lane = Lane:new("Lane Long Tag", laneSignal)

            lane:vehicleEntered(firstName)
            lane:vehicleEntered(secondName)
            lane:vehicleEntered(thirdName)
            lane:resetWaitCount()
            ...
            assert.is_truthy(string.find(tagText, secondName, 1, true))
            assert.is_nil(string.find(tagText, thirdName, 1, true))
            assert.equals(3, lane.queue:size())
            assert.equals(firstName, lane.queue:firstElement())
        end)
        it("loads a shortened lane queue with generic fallback vehicles", function ()
            local firstName = "#Car1"
            EEPSignalSetTagText(170, "f=3,p=Rot,q=" .. firstName .. ",w=0,")
            local laneSignal = TrafficLight:new("K5", 170, TrafficLightModel.Unsichtbar_2er)

            local lane = Lane:new("Lane Short Tag Load", laneSignal)

            assert.equals(3, lane.queue:size())
            assert.equals(firstName, lane.queue:firstElement())
            assert.are.same({ firstName, "train 2", "train 3" }, lane.queue:elements())
        end)
        it("removes one queued vehicle when a leave contact has no train name", function ()
            EEPSignalSetTagText(171, "f=3,p=Rot,q=#Car1|#Car2|#Car3,w=0,")
            ...
            assert.equals(3, lane.vehicleCount)
            assert.equals(3, lane.queue:size())
            assert.are.same({ "#Car2", "#Car3", "#Car4" }, lane.queue:elements())
        end)
        it("keeps the first known train when an older unknown train leaves", function ()
            local laneSignal = TrafficLight:new("K7", 172, TrafficLightModel.Unsichtbar_2er)
            local lane = Lane:new("Lane Unknown Leave Name", laneSignal)

            lane:vehicleEntered("#Car1")
            lane:vehicleLeft("#Ghost")
            lane:vehicleEntered("#Car2")

            assert.equals(2, lane.vehicleCount)
            assert.equals(2, lane.queue:size())
            assert.are.same({ "#Car1", "#Car2" }, lane.queue:elements())
        end)
        it("trims stale loaded queue entries instead of increasing the loaded count", function ()
            EEPSignalSetTagText(173, "f=1,p=Rot,q=#Old|#Car1,w=0,")
            local laneSignal = TrafficLight:new("K8", 173, TrafficLightModel.Unsichtbar_2er)
            local lane = Lane:new("Lane Stale Tag Load", laneSignal)

            assert.equals(1, lane.vehicleCount)
            assert.equals(1, lane.queue:size())
            assert.are.same({ "#Car1" }, lane.queue:elements())
        end)
        it("trims stale runtime queue entries on enter instead of increasing the count", function ()
            local Queue = require("ce.hub.util.Queue")
            local laneSignal = TrafficLight:new("K9", 174, TrafficLightModel.Unsichtbar_2er)
            local lane = Lane:new("Lane Stale Runtime Queue", laneSignal)
            lane.queue = Queue:new()
            lane.queue:push("#Old")
            lane.queue:push("#Car1")
            lane.vehicleCount = 1
