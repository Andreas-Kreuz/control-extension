insulate("ce.mods.road.SignalGroup", function ()
    ...
    it("adds every signal head from a group to a traffic phase", function ()
        ...
        local phase = TrafficPhase:new("P1"):addSignalGroups(sg)
    ...
    it("stores import metadata for approach and turn directions", function ()
        local Lane = require("ce.mods.road.Lane")
        local SignalGroup = require("ce.mods.road.SignalGroup")

        local sg = SignalGroup:new("sgLane1Left")
            :setApproach(Lane.Approach.WEST)
            :setTurnDirections(Lane.Directions.LEFT, Lane.Directions.HALF_LEFT)

        assert.equals(Lane.Approach.WEST, sg:getApproach())
        assert.are.same({ Lane.Directions.LEFT, Lane.Directions.HALF_LEFT }, sg:getTurnDirections())
    end)
    it("stores script variable metadata", function ()
        local SignalGroup = require("ce.mods.road.SignalGroup")
        local sg = SignalGroup:new("Lane 1 left")

        assert.equals(sg, sg:setScriptVariableName("sgLane1Left"))
        assert.equals("sgLane1Left", sg:getScriptVariableName())
        assert.equals(sg, sg:scriptVariableName("sgLane1LeftAlias"))
        assert.equals("sgLane1LeftAlias", sg:getScriptVariableName())
    end)
    ...
    it("asserts a vehicle and pedestrian release on the same physical signal in one phase", function ()
        ...
        local K1 = TrafficLight:new("K1", -1, TrafficLightModel.NONE):asPedestrianSignal("F1")
