...
BusHS_Tram_Info_6_RG3.initStation = function (displayStructure, stationName, platform, routeNames)
    ...
    StructureTextureTextCache.set(displayStructure, 1, stationName)
    if (routeNames) then
        for i = 1, 5 do
            local route = routeNames[i]
            local index = i + 1
            StructureTextureTextCache.set(displayStructure, index, route or "")
        end
    end
    StructureTextureTextCache.set(displayStructure, 7, "Steig " .. platform)
end
