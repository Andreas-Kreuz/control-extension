    ...
    for scheduledTask in pairs(scheduledTasks) do
        if previousTask == scheduledTask then
            ...
            if Scheduler.debug then
                print(string.format(
                    "[#Scheduler] Task scheduled: '%s' (%s seconds after '%s')",
                    newTask.name,
                    offsetInSeconds,
                    previousTask.name
                ))
    ...
    if self.ready then
        ...
        for currentTask, plannedAtSeconds in pairs(self.scheduledTasks) do
            ...
            if secondsSinceMidnight >= plannedAtSeconds then
                if Scheduler.debug then
                    print(string.format(
                        "[#Scheduler %s] Running Task: '%s' (NOW)",
        ...
        for currentTask in pairs(scheduledTasks) do
            ...
            for successorAction, offsetSeconds in pairs(currentTask.subsequentTask) do
                if Scheduler.debug then
                    print(string.format(
                        "[#Scheduler %s] Scheduling Task: '%s' in %s seconds (at %s)",
                        secondsSinceMidnight,
                        successorAction.name,
                        offsetSeconds,
                        currentSecondsSinceMidnight() + offsetSeconds
                    ))
                end
...
---@param precedingTask? Task optional - when given, the new action is scheduled after the delayInSeconds
-- of the preceding task have passed
function Scheduler:scheduleTask(offsetInSeconds, newTask, precedingTask)
    ...
    if precedingTask then
        ...
        if not previousTaskFound then
            print(string.format(
                "[#Scheduler] DID NOT FIND PREDECESSOR TASK FOR! : %s --> %s",
                precedingTask.name,
                newTask.name
            ))
        end
    ...
    if not previousTaskFound and not self.scheduledTasks[newTask] then
        ...
        if Scheduler.debug then
            print(string.format(
                "[#Scheduler] Task scheduled: '%s' in %s seconds (at %s)",
                newTask.name,
                offsetInSeconds,
